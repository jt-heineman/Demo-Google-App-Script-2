...
//var PasteSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('1. Data Summary');
...
var app = SpreadsheetApp;
var ss = app.getActiveSpreadsheet().getSheetByName("Intake");
var lr = ss.getLastRow();

  for (var i = 2;i<=lr;i++ ){
  
  var Project = ss.getRange(i, 2).getValue();
  var Check = ss.getRange(i, 22).getValue();
  var Title = "Project Created: " + Project ;
  var Comment = "For your acknowledgement, a project was created, please verify if all is accordingly on the Budget Tracker"

  //if(Check ===  '' && Check2 !='' && CurrentPrice > 0){
      
    ss.getRange(i, 22).setValue("TRUE");
